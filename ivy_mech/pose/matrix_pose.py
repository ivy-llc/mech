...
def euler_pose_to_mat_pose(euler_pose, convention='zyx', batch_shape=None):
    ...
    # BS x 3 x 4
    return _ivy.concat([rot_mat, _ivy.expand_dims(euler_pose[..., 0:3], -1)], axis=-1)
...
def rot_vec_pose_to_mat_pose(rot_vec_pose):
    ...
    # BS x 3 x 4
    return _ivy.concat([rot_mat, _ivy.expand_dims(rot_vec_pose[..., 0:3], -1)], axis=-1)
...
def axis_angle_pose_to_mat_pose(axis_angle_pose):
    ...
    # BS x 3 x 4
    return _ivy.concat(
        [rot_mat, _ivy.expand_dims(axis_angle_pose[..., :3], -1)], axis=-1)
