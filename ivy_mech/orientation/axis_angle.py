...
def rot_mat_to_axis_angle(rot_mat,  dev=None):
    """
    ...
    return quaternion_to_axis_angle(quat, dev)


def euler_to_axis_angle(euler_angles, convention='zyx', batch_shape=None, dev=None):
    """
...
def quaternion_to_axis_angle(quaternion, dev=None):
    """
    ...
    if dev is None:
        dev = _ivy.get_device(quaternion)
    ...
    vector_x = _ivy.where(theta != 0, e1 / (_ivy.sin(theta / 2) + MIN_DENOMINATOR), _ivy.zeros_like(theta, dev=dev))
    vector_y = _ivy.where(theta != 0, e2 / (_ivy.sin(theta / 2) + MIN_DENOMINATOR), _ivy.zeros_like(theta, dev=dev))
    vector_z = _ivy.where(theta != 0, e3 / (_ivy.sin(theta / 2) + MIN_DENOMINATOR), _ivy.zeros_like(theta, dev=dev))

    # BS x 4
    return _ivy.concatenate((vector_x, vector_y, vector_z, theta), -1)


def quaternion_to_polar_axis_angle(quaternion, dev=None):
...
def quaternion_to_rotation_vector(quaternion, dev=None):
