...
class PoseTestData(OrientationTestData):
    ...
    def __init__(self):
        ...
        # axis-angle pose
        self.axis_angle_pose = np.concatenate((self.cartesian_coords, self.axis_angle), 1)
        self.batched_axis_angle_pose = np.expand_dims(self.axis_angle_pose, 0)

        # rotation vector pose
        self.rot_vec_pose = np.concatenate((self.cartesian_coords, self.rotation_vector), 1)
        ...
        # matrix pose
        self.matrix_pose = np.concatenate((self.rotation_matrix, np.expand_dims(self.cartesian_coords, -1)), 2)
        self.batched_matrix_pose = np.expand_dims(self.matrix_pose, 0)

        # quaternion pose
        self.quaternion_pose = np.concatenate((self.cartesian_coords, self.quaternion), 1)
        ...
        self.incremented_quaternion = np.concatenate((self.quaternion_pose[:, 0:3] +
                                                      self.velocity[:, 0:3] * self.control_dt, new_quaternion), -1)
        self.batched_incremented_quaternion = np.expand_dims(self.incremented_quaternion, 0)
