...
def test_make_coordinates_homogeneous(device, fw):
    ivy.set_backend(fw)
    assert np.allclose(ivy_mech.make_coordinates_homogeneous(ptd.cartesian_coords),
                       ptd.cartesian_coords_homo, atol=1e-6)
    assert np.allclose(ivy_mech.make_coordinates_homogeneous(ptd.batched_cartesian_coords),
...
def test_make_transformation_homogeneous(device, fw):
    ivy.set_backend(fw)
    assert np.allclose(ivy_mech.make_transformation_homogeneous(ptd.ext_mat), ptd.ext_mat_homo, atol=1e-6)
    assert np.allclose(ivy_mech.make_transformation_homogeneous(ptd.batched_ext_mat),
