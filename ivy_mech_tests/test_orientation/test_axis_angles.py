...
def test_rot_mat_to_axis_angle(dev_str, call):
...
def test_euler_to_axis_angle(dev_str, call):
...
def test_quaternion_to_vector_and_angle(dev_str, call):
...
def test_quaternion_to_polar_axis_angle(dev_str, call):
...
def test_quaternion_to_rotation_vector(dev_str, call):
    assert np.allclose(call(ivy_mech.quaternion_to_rotation_vector, otd.quaternion, dev_str='cpu'),
                       otd.rotation_vector, atol=1e-6)
    assert np.allclose(call(ivy_mech.quaternion_to_rotation_vector, otd.batched_quaternion, dev_str='cpu')[0],
                       otd.rotation_vector, atol=1e-6)


def test_get_random_axis_angle(dev_str, call):
