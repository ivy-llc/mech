...
def test_vector_and_angle_to_quaternion():
...
def test_rot_mat_to_quaternion():
    for lib, call in helpers.calls:
        if call is helpers.mx_graph_call:
            # mxnet symbolic does not fully support array slicing
            continue
...
def test_euler_to_quaternion():
    for lib, call in helpers.calls:
        ...
        for conv in ivy_mech.VALID_EULER_CONVENTIONS:
            quaternion = ivy_mech.euler_to_quaternion(qtd.euler_angles, conv, f=helpers._ivy_np)
...
def test_rotation_vector_to_quaternion():
    for lib, call in helpers.calls:
        if call is helpers.mx_graph_call:
            # mxnet symbolic does not fully support array slicing
            continue
...
def test_inverse_quaternion():
    for lib, call in helpers.calls:
        ...
        assert np.allclose(call(ivy_mech.inverse_quaternion,
                                call(ivy_mech.inverse_quaternion, qtd.quaternion, f=lib), f=lib),
                           qtd.quaternion, atol=1e-6)


def test_get_random_quaternion():
    for lib, call in helpers.calls:
        if call is helpers.mx_graph_call:
            # mxnet symbolic does not fully support array slicing
            continue
        assert call(ivy_mech.get_random_quaternion, f=lib).shape == (4,)
        assert call(ivy_mech.get_random_quaternion, batch_shape=(1, 1), f=lib).shape == (1, 1, 4)


def test_scale_quaternion_theta():
    for lib, call in helpers.calls:
        if call is helpers.mx_graph_call:
            # mxnet symbolic does not fully support array slicing
            continue
